...
function PlanCardBody({
  ...
  return (
    <>
      <div className={cn("relative z-10 mb-6", isFeatured && "mt-2")}>
        {plan.logoDataUrl && (
      ...
      <button
        ...
        onClick={onSelect}
        className={cn(
          "relative z-10 w-full rounded-2xl py-3.5 text-base font-semibold transition-all duration-300 active:scale-[0.98]",
...
export function PlanCard({
    ...
    return (
      <div className="landing-plan-featured-shell relative hover:-translate-y-1 hover:scale-[1.02]">
        <div className="absolute left-1/2 top-0 z-30 -translate-x-1/2 -translate-y-1/2">
          <span className="theme-badge-popular inline-flex rounded-full px-4 py-1.5 font-bold text-white">
            {copy.planBadgePopular}
          </span>
        </div>
        <div className="landing-plan-featured-inner landing-plan-card landing-plan-card-featured relative flex flex-col p-7 pt-8 sm:p-8 sm:pt-9">
  ...
  return (
    <div
      className={cn(
        "landing-card-surface landing-plan-card relative flex flex-col rounded-[1.75rem] p-7 sm:p-8",
